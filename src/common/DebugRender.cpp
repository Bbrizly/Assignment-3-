...
DebugRender::DebugRender() {
    ...
    m_pProgram = wolf::ProgramManager::CreateProgram("data/Debug.vsh", "data/Debug.fsh");
...
void DebugRender::UpdateBuffer(const std::vector<float>& vertexData) {
    ...
    if (dataSize > 0) {
        m_vertexBuffer = wolf::BufferManager::CreateVertexBuffer(vertexData.data(), dataSize);
        if (m_vertexDecl) {
            delete m_vertexDecl;
            m_vertexDecl = nullptr;
        }
        ...
        m_vertexDecl->Begin();
        m_vertexDecl->AppendAttribute(wolf::AT_Position, 3, wolf::CT_Float);
        m_vertexDecl->AppendAttribute(wolf::AT_Color, 3, wolf::CT_Float);
